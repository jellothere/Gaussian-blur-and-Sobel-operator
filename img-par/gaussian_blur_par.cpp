...
vector<u_int8_t> gaussian_blur_par(vector<u_int8_t> initial_image, int width, int height){
    ...
    #pragma omp parallel for reduction(+:red,green,blue) collapse(2) num_threads(16)
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j += 3) {
                ...
                //Setting the color accumulators to 0
                blue = 0;
                green = 0;
                red = 0;
            }
